    ...
    match inputtype {
        MouseInputType::LeftDown => {
            if gui.preference.keywindow.is_open {
                let click_button = gui.preference.keywindow.click_buttons(systems, screen_pos);
            ...
            if !gui.preference.scrollbar.in_hold {
                let click_button = gui.preference.click_buttons(systems, screen_pos);
                if let Some(index) = click_button {
                    match index {
                        0 => {
                ...
                match gui.preference.selected_menu {
                    PREF_TAB_GENERAL => {
                        if let Some(index) = gui.preference.is_coloreditor_open {
                            if gui.preference.in_color_selection(systems, screen_pos) {
                                gui.preference.select_text(systems, screen_pos);
                                if gui
                                    .preference
                                    .click_color_selection_button(systems, screen_pos)
                                {
                                    if let SettingData::ColorSelection(colorselection) =
                                        &mut gui.preference.setting_data[index]
                                    {
                                        let data = colorselection.color_editor.data;
                                        systems.gfx.set_color(
                                        ...
                                        match index {
                                            GENERAL_MAPCOLOR => {
                                                config_data.map_selection_color = data
                                            }
                                            GENERAL_TILECOLOR => {
                                                config_data.tile_selection_color = data
                                            }
                                            _ => {}
                                        }
                                    }
                            ...
                            match &mut gui.preference.setting_data[config_index] {
                                SettingData::Checkbox(checkbox) => {
                                    ...
                                    match config_index {
                                        ...
                                        GENERAL_HIDETILEBG => {
                                            ...
                                            systems
                                                .gfx
                                                .set_visible(gui.bg_layout[2], !checkbox.is_select);
                                        }
                                        GENERAL_HIDEMAPBG => {
                                            config_data.hide_mapview_bg = checkbox.is_select;
                                            systems
                                                .gfx
                                                .set_visible(gui.bg_layout[1], !checkbox.is_select);
                                        }
                                        _ => {}
                                    }
                                    gui.preference.hide_color_selection(systems);
                                ...
                                SettingData::ColorSelection(colorselection) => {
                                    if gui.preference.is_coloreditor_open.is_none() {
                                        colorselection.open_color_editor(systems);
                    ...
                    PREF_TAB_KEYBIND => {
                        if let Some(key_index) = gui.preference.select_keylist(systems, screen_pos)
                        {
...
pub fn preference_key_input(systems: &mut DrawSetting, event: &KeyEvent, gui: &mut Interface) {
                    ...
                    &mut gui.preference.setting_data[index]
                {
                    if colorselection.color_editor.is_open {
                        ...
                        let value = colorselection.color_editor.textbox
                            ...
                            .unwrap_or_default()
                            .min(255);
                        colorselection.color_editor.data[gui.preference.editing_index] =
                            value as u8;
                    }
                }
            }
