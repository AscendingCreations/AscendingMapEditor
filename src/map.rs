...
use crate::{collection::*, create_basic_label, map_data::*, ConfigData, DrawSetting};
...
impl MapView {
    pub fn new(systems: &mut DrawSetting, config_data: &mut ConfigData) -> Self {
        ...
        for i in 0..1024 {
            ...
            let (image, text) = (systems.gfx.add_rect(img, 0), systems.gfx.add_text(txt, 1));
            ...
            for (_key, changedata) in data.changes.iter() {
                ...
                match changedata.record_type {
                    ...
                    RecordType::Attribute => {
                        ...
                        let attribute_enum =
                            MapAttribute::convert_to_enum(changedata.id as u32, &changedata.data);
                        let tilenum = get_tile_pos(pos.x as i32, pos.y as i32);
                    ...
                    RecordType::Zone => {
                        ...
                        let does_exist = if self.map_zone_loc[zone_index]
                            .pos
                            .iter()
                            .any(|&check_pos| check_pos == Vec2::new(pos.x, pos.y))
                        {
                            1
                        } else {
                            0
                        ...
                        if changedata.id > 0 {
                            ...
                            self.map_zone_loc[zone_index]
                                .pos
                                .retain(|&check_pos| check_pos != Vec2::new(pos.x, pos.y));
                        }
                    }
                }
            }
