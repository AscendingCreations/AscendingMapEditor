...
use crate::{collection::*, config, map::*, tileset::*, ConfigData, DrawSetting, GameInput};
...
impl Interface {
    ...
    pub fn hover_tool_button(&mut self, systems: &mut DrawSetting, mouse_pos: Vec2) {
    ...
    pub fn set_tool(&mut self, systems: &mut DrawSetting, tool_index: usize) {
        if self.current_tool != tool_index {
            self.buttons[self.current_tool].set_state(systems, ButtonState::Normal);
    ...
    pub fn hover_selectionbox(&mut self, systems: &mut DrawSetting, mouse_pos: Vec2) {
        // We check if buttons are within the mouse position
    ...
    pub fn hover_tab_option(&mut self, systems: &mut DrawSetting, mouse_pos: Vec2) {
    ...
    pub fn select_tab_option(&mut self, systems: &mut DrawSetting, tab_index: usize) {
    ...
    pub fn update_scroll(&mut self, systems: &mut DrawSetting, cur_value: usize) {
    ...
    pub fn select_textbox(&mut self, systems: &mut DrawSetting, mouse_pos: Vec2) {
    ...
    pub fn get_attribute_setting(&mut self) -> MapAttribute {
        ...
        match attribute {
            MapAttribute::Warp(_) => {
                let (mut mx, mut my, mut mg, mut tx, mut ty) = (0_i32, 0_i32, 0_u64, 0_u32, 0_u32);
