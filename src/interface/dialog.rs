...
impl Dialog {
    pub fn new(
        ...
        let msg = match dialog_type {
            DialogType::ExitConfirm => "Are you sure that you want to close the editor?",
            DialogType::MapSave => "Would you like to save the changes to the following map/s?",
            DialogType::MapLoad => "Please enter the map location that you would like to load",
            _ => "Error",
        };
        ...
        let content_image = match dialog_type {
            DialogType::MapSave => {
                ...
                let label_box_pos = Vec2::new(
                    window_pos.x + ((window_size.x * 0.5) - (label_box_size.x * 0.5)).floor(),
                    window_pos.y + 65.0,
                );
        ...
        let content_text = match dialog_type {
            DialogType::MapSave => {
                ...
                for index in 0..4 {
                    ...
                    let content_pos = Vec2::new(
                        window_pos.x + ((window_size.x * 0.5) - (label_size.x * 0.5)).floor(),
            ...
            DialogType::MapLoad => {
                ...
                let content_pos = Vec2::new(
                    window_pos.x + ((window_size.x * 0.5) - (textbox_total_size * 0.5)),
                    window_pos.y + 66.0,
                )
                .floor();
