...
impl TilesetList {
    pub fn new(systems: &mut DrawSetting) -> Self {
        ...
        let max_view =
            std::cmp::min(systems.resource.tilesheet.len() as u32, MAX_VISIBLE_LIST) as usize;
        ...
        let scrollbar_value = max_tileset.max(MAX_VISIBLE_LIST) - MAX_VISIBLE_LIST;
        ...
        let bg = vec![systems.gfx.add_rect(bg1, 0), systems.gfx.add_rect(bg2, 0)];
    ...
    pub fn select_list(&mut self, systems: &mut DrawSetting, mouse_pos: Vec2) -> bool {
        if !self.visible {
            return false;
        }
        if let Some(index) = self.selection_buttons.iter().position(|button| {
            ...
            if self.selected_tileset != tileset_index {
                if let Some(view_index) = self.view_index {
                    self.selection_buttons[view_index].set_select(systems, false);
                }
    ...
    pub fn hover_selection(&mut self, systems: &mut DrawSetting, mouse_pos: Vec2) {
        if !self.visible {
            return;
        }
