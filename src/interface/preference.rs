...
use crate::{
    collection::*,
    config::*,
    interface::{button::*, checkbox::*, color_selection::*, label::*, scrollbar::*},
    DrawSetting,
};
...
impl Preference {
                        ...
                        &mut self.setting_data[index]
                    {
                        if colorselection.color_editor.is_open {
                            ...
                            if (mouse_pos.x) >= pos.x
                                ...
                                && (mouse_pos.y) <= pos.y + size.y
                            {
                                colorselection.color_editor.button.set_hover(systems, true);
                            } else {
                                colorselection.color_editor.button.set_hover(systems, false);
                            }
                        }
                    }
                }
        ...
        if self.selected_menu == PREF_TAB_GENERAL {
            if let Some(index) = self.is_coloreditor_open {
                if let SettingData::ColorSelection(colorselection) = &mut self.setting_data[index] {
                    if colorselection.color_editor.is_open {
                        colorselection.color_editor.button.set_click(systems, false);
                    }
                }
            }
        }
            ...
            if let SettingData::ColorSelection(colorselection) = &mut self.setting_data[index] {
                if colorselection.color_editor.is_open {
                    ...
                    if (mouse_pos.x) >= pos.x
                        ...
                        && (mouse_pos.y) <= pos.y + size.y
                    {
                        colorselection.color_editor.button.set_click(systems, true);
    ...
    pub fn select_menu_button(&mut self, systems: &mut DrawSetting, mouse_pos: Vec2) -> bool {
        if self.keywindow.is_open {
            return false;
        }
        let mut found_button = None;
    ...
    pub fn select_keylist(&mut self, systems: &mut DrawSetting, mouse_pos: Vec2) -> Option<usize> {
        if self.keywindow.is_open {
            return None;
        }
        let mut found_button = None;
    ...
    pub fn select_config(&mut self, systems: &mut DrawSetting, mouse_pos: Vec2) -> Option<usize> {
        let mut found_button = None;
    ...
    pub fn reset_preference(&mut self, systems: &mut DrawSetting, config_data: &mut ConfigData) {
        // Reset data
        ...
        if let Some(config_index) = self.is_coloreditor_open {
                ...
                &mut self.setting_data[config_index]
            {
                ...
                if selected_index < 0 {
                    ...
                    colorselection.color_editor.textbox[last_selected].set_select(systems, false);
                }
    ...
    pub fn in_color_selection(&mut self, systems: &mut DrawSetting, mouse_pos: Vec2) -> bool {
...
pub fn open_preference_tab(
    ...
    preference
        ...
        .for_each(|data| match data {
            ...
            SettingData::ColorSelection(colorselection) => colorselection.unload(systems),
            SettingData::Label(_label) => {}
            _ => {}
        });
        ...
        PREF_TAB_GENERAL => {
            ...
            preference.setting_data = vec![
                ...
                SettingData::ColorSelection(ColorSelection::new(
                    systems,
                    Vec3::new(pos.x, pos.y - 72.0, ORDER_PREFERENCE_SETTING_IMG1),
                    Vec2::new(70.0, 20.0),
                ...
                SettingData::ColorSelection(ColorSelection::new(
                    systems,
                    Vec3::new(pos.x, pos.y - 96.0, ORDER_PREFERENCE_SETTING_IMG1),
                    Vec2::new(70.0, 20.0),
