...
impl Textbox {
    ...
    pub fn new(
        ...
        let rect = Rect::new(
            ...
            (size * if can_scale { systems.scale as f32 } else { 1.0 }).floor(),
            bg_color,
            order[0],
        );
        let bg = systems.gfx.add_rect(rect, bg_layer, "Textbox BG", false);
        ...
        let select_rect = Rect::new(
            ...
            Vec2::new(
                0.0,
                size.y * if can_scale { systems.scale as f32 } else { 1.0 },
            )
            .floor(),
            selection_bg_color,
            order[1],
        );
        let selection = systems
            .gfx
            .add_rect(select_rect, bg_layer, "Textbox Select", visible);

        let mut text_data = create_label(
            systems,
            Vec3::new(b_pos.x, b_pos.y, base_pos.z),
            size * if can_scale { systems.scale as f32 } else { 1.0 },
            Bounds::new(
                b_pos.x,
                b_pos.y,
                b_pos.x + (size.x * if can_scale { systems.scale as f32 } else { 1.0 }),
                b_pos.y + (size.y * if can_scale { systems.scale as f32 } else { 1.0 }),
            ),
            text_color,
            order[2],
            16.0,
        ...
        let text_index = systems
            .gfx
            .add_text(text_data, text_layer, "Textbox Text", visible);

        let caret_rect = Rect::new(
            &mut systems.renderer,
            Vec3::new(b_pos.x, b_pos.y, base_pos.z),
            (Vec2::new(2.0, size.y) * if can_scale { systems.scale as f32 } else { 1.0 }).floor(),
            text_color,
            order[2],
        );
        let caret = systems
            .gfx
            .add_rect(caret_rect, bg_layer, "Textbox Caret", false);
    ...
    pub fn set_z_order(&mut self, systems: &mut SystemHolder, z_order: f32) {
        ...
        systems.gfx.set_pos(
            &self.text_index,
            Vec3::new(b_pos.x + self.adjust_x, b_pos.y, self.base_pos.z),
        );
        systems.gfx.set_pos(
            &self.caret,
            Vec3::new(b_pos.x + self.caret_left, b_pos.y, self.base_pos.z),
        );
        systems.gfx.set_pos(
            &self.selection,
            Vec3::new(b_pos.x + self.selection_pos, b_pos.y, self.base_pos.z),
        );
    }
    ...
    pub fn set_text(&mut self, systems: &mut SystemHolder, text: String) {
        ...
        //    let size =
        //        measure_string(systems, &char.to_string(), self.can_scale).x;
        //    old_size.push(size);
        //}

        let char_size: Vec<f32> = measure_character(systems, &msg, self.can_scale)
            .iter()
            .map(|data| data.x)
            .collect();

        //for char in char_size.iter().rev() {
        //    self.char_size.insert(self.caret_pos, *char);
        //}

        self.char_size.reserve(char_size.len());
        let mut v = self.char_size.split_off(self.caret_pos);
    ...
    pub fn enter_text(
        ...
        match key {
            ...
            Key::Named(NamedKey::Enter) | Key::Character('\r') => {
                return;
            }
            _ => {}
        }
        ...
        if let Key::Named(NamedKey::Delete) = key {
            if self.hold_initial_index == self.hold_final_index {
                self.text = remove_text(self.text.clone(), self.caret_pos);
                self.data_text = remove_text(self.text.clone(), self.caret_pos);
            } else {
        ...
        if self.special_key_hold[KEY_CTRL] {
            match key {
                ...
                Key::Character('v') => {
                    ...
                    if self.data_text.chars().count() + clipboard_text.chars().count() >= self.limit
                    {
                        return;
                    }
                    ...
                    if can_proceed {
                        ...
                        self.text = insert_text(self.text.clone(), self.caret_pos, &clipboard_text);
                        ...
                        self.data_text =
                            insert_text(self.data_text.clone(), self.caret_pos, &clipboard);

                        let char_size: Vec<f32> =
                            measure_character(systems, &clipboard, self.can_scale)
                                .iter()
                                .map(|data| data.x)
                                .collect();
                        ...
                        self.move_caret_pos(systems, false, clipboard_text.chars().count(), false);
            ...
            match key {
                Key::Named(NamedKey::Backspace) => {
                    if self.hold_initial_index != self.hold_final_index {
                        ...
                        self.move_caret_pos(systems, true, 1, true);
                        self.text = remove_text(self.text.clone(), self.caret_pos);
                        self.data_text = remove_text(self.data_text.clone(), self.caret_pos);
                ...
                _ => {
                    ...
                    if let Some(char) = key_char {
                        ...
                        if can_proceed {
                            ...
                            let msg = if self.hide_content { '*' } else { char };

                            self.text = insert_char(self.text.clone(), self.caret_pos, char);
                            self.data_text =
                                insert_char(self.data_text.clone(), self.caret_pos, msg);
                            let size = measure_string(
                                systems,
                                &msg.to_string(),
                                self.can_scale,
    ...
    pub fn move_caret_pos(
        ...
        let char_count = self.data_text.chars().count();
        ...
        let (start, end) = if move_left {
            ...
            self.caret_pos = self.caret_pos.saturating_add(count).min(char_count);
            (start, self.caret_pos)
        };

        let edit_text_whole: String = self.data_text.chars().take(end).collect();
        let edit_text_part: String = self.data_text.chars().take(start).collect();

        let size_whole = measure_string(systems, &edit_text_whole, self.can_scale, 16.0, 16.0).x;
        let size_part = measure_string(systems, &edit_text_part, self.can_scale, 16.0, 16.0).x;
        ...
        if remove_content {
            ...
            let total_size = measure_string(systems, &text, self.can_scale, 16.0, 16.0).x;
            if total_size
                > (self.size.x
                    * if self.can_scale {
                        systems.scale as f32
    ...
    pub fn select_text(&mut self, systems: &mut SystemHolder, screen_pos: Vec2) {
        let b_pos = Vec2::new(self.base_pos.x, self.base_pos.y)
            + (self.adjust_pos
                * if self.can_scale {
                    systems.scale as f32
        ...
        if let Some(index) = found_index {
            ...
            match offset.cmp(&0) {
                std::cmp::Ordering::Less => {
                    ...
                    self.move_caret_pos(systems, true, move_count as usize, false);
                }
    ...
    pub fn hold_move(&mut self, systems: &mut SystemHolder, screen_pos: Vec2) {
        ...
        let last_pos = if let Some(last_pos) = self.char_pos.last() {
            if let Some(last_size) = self.char_size.last() {
                ...
                if char_x_pos >= b_pos.x && char_x_pos + *last_size <= b_pos.x + self.size.x {
                    char_x_pos
                } else {
                    b_pos.x + self.size.x
                }
    ...
    pub fn update_selection(&mut self, systems: &mut SystemHolder) {
        ...
        let size = if self.hold_initial_index != self.hold_final_index && !self.data_text.is_empty()
        {
            let first = cmp::min(self.hold_initial_index, self.hold_final_index);
            let second = cmp::max(self.hold_initial_index, self.hold_final_index);
            ...
            self.selection_pos = (self.adjust_x + self.char_pos[first]).floor().max(0.0);
            measure_string(systems, &text, self.can_scale, 16.0, 16.0)
                .x
                .floor()
                .min(
                    (self.size.x
    ...
    pub fn in_area(&self, systems: &mut SystemHolder, screen_pos: Vec2) -> bool {
        let b_pos = Vec2::new(self.base_pos.x, self.base_pos.y)
            + (self.adjust_pos
                * if self.can_scale {
                    systems.scale as f32
...
pub fn replace_text(text: String, from_pos: usize, to_pos: usize, replace_to: String) -> String {
    let mut first_text = String::new();
    let mut second_text = String::new();
    for (cur_pos, char) in text.chars().enumerate() {
        if cur_pos < from_pos {
...
pub fn is_numeric(char: &str) -> bool {
    char.trim().parse::<i64>().is_ok() || char.trim().parse::<f64>().is_ok()
}
